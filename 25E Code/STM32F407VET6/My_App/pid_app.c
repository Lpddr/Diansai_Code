#include "pid_app.h"
extern float Chassis_Feedforward_Speed;
...
// 前馈力度系数
float Gimbal_FF_K = 0.81f;
// IMU 阻尼系数 (新增)
float Gimbal_Gyro_K = 0.22f;
...
static void Pid_Mode_0_Handler(void)
{
    ...
    // [前馈融合] 视觉输出 - (底盘转速前馈 + IMU陀螺仪阻尼)
    X_Output -= (Chassis_Feedforward_Speed * Gimbal_FF_K); // 底盘补偿
    X_Output -= (g_imu_data.gyro.z * Gimbal_Gyro_K);       // IMU 阻尼
