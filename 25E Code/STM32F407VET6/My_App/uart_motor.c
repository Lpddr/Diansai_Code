...
#include <string.h> // [新增] 必须加这个，不然 memcpy 用不了

// 存放底盘发过来的前馈速度
float Chassis_Feedforward_Speed = 0.0f;
...
{
	// 前馈数据解析
	// 协议格式: AA [float 4字节] BB
	if ((uint8_t)data[0] == 0xAA)
	{
		float received_speed = 0;
		memcpy(&received_speed, &data[1], 4);

		Chassis_Feedforward_Speed = received_speed;
		return;
	}
